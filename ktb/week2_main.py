...
while(True):
    ...
    if i == 1:
        #작성
        week2_utils.create_f(i,"","")
    elif i == 2:
        #조회
        print(r)
        try:
            title,content = week2_utils.read_f(r)
            print(f'Title : {title}\n{content}')#내용 출력
        except Exception as error:
            print(error)
            continue
        ...
        print(u)
        try:
            title, content = week2_utils.read_f(u) #읽어올 번호 입력받아 title content return
        except Exception as error:
            print(error)
            continue
        content = week2_utils.write_c() #새로 입력받기
        week2_utils.create_f(i, title, content)
        
    elif i == 4:
        #삭제
        print(d)
        try:
            title, content = week2_utils.read_f(d)
        except Exception as error:
            print(error)
            continue
        week2_utils.create_f(i,title, content)
